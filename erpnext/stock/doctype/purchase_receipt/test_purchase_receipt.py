...
class TestPurchaseReceipt(FrappeTestCase):
	...
	def test_pr_ignore_pricing_rule_TC_B_050(self):
		...
		# Ensure the Item exists, else create it
		if not frappe.db.exists("Item", item_code):
			frappe.get_doc({
				"doctype": "Item",
				"item_code": item_code,
		...
		# Ensure the Item Price exists, else create it
		...
		# Ensure the Pricing Rule exists, else create it
		...
		pr.insert()
		# Validate rate and discount before saving
		self.assertEqual(len(pr.items), 1)
		self.assertEqual(pr.items[0].rate, 117)  # Rate after applying Pricing Rule
		self.assertEqual(pr.items[0].discount_percentage, 10)

		# Apply "Ignore Pricing Rule" and save
		pr.ignore_pricing_rule = 1
		pr.save()

		# Validate rate and discount after ignoring Pricing Rule
		self.assertEqual(pr.items[0].rate, 130)  # Original rate from Item Price
		self.assertEqual(pr.items[0].discount_percentage, 0)  # No discount applied
		...
		self.assertEqual(pr.items[0].rate, 130)  # Ensure rate remains unchanged after submission
