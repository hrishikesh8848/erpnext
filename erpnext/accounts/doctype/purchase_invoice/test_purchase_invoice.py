...
class TestPurchaseInvoice(FrappeTestCase, StockTestMixin):
	...
	def test_pi_ignore_pricing_rule_TC_B_051(self):
		...
		self.assertEqual(pi.items[0].rate, 117)  # Rate after applying Pricing Rule
		self.assertEqual(pi.items[0].discount_percentage, 10)

		pi.ignore_pricing_rule = 1
		pi.save()

		self.assertEqual(pi.items[0].rate, 130)  # Original rate from Item Price
		self.assertEqual(pi.items[0].discount_percentage, 0)  # No discount applied
