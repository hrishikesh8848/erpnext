...
def validate_fiscal_year(date, fiscal_year, company, label="Date", doc=None):
    ...
    if fiscal_year not in years:
        if doc:
            ...
            throw(
                _("{0} '{1}' not in Fiscal Year {2}").format(
                    label, formatdate(date), fiscal_year
                )
            )
...
def get_count_on(account, fieldname, date):
    ...
    if account:
        ...
        if acc.report_type == "Profit and Loss":
            cond.append(
                "posting_date >= '%s' and voucher_type != 'Period Closing Voucher'"
                % year_start_date
            )
        ...
        for gle in entries:
            if fieldname not in ("invoiced_amount", "payables"):
                ...
                if (
                    ...
                    outstanding_amount = (
                        flt(gle.get(dr_or_cr)) - flt(gle.get(cr_or_dr)) - payment_amount
                    )
...
def reconcile_against_document(
    ...
    for key, entries in reconciled_entries.items():
        ...
        for entry in entries:
            ...
            dimensions_dict = _build_dimensions_dict_for_exc_gain_loss(
                entry, active_dimensions
            )
        ...
        if (
            ...
            and doc.book_advance_payments_in_separate_party_account
        ):
            ...
            create_payment_ledger_entry(
                gl_map, update_outstanding="No", cancel=0, adv_adj=1, reconcile=True
            )
...
def validate_allocated_amount(args):
    ...
    elif flt(args.get("allocated_amount"), precision) > flt(
        args.get("unadjusted_amount"), precision
    ):
...
def update_reference_in_journal_entry(d, journal_entry, do_not_save=False):
    ...
    new_row.set(
        "credit" if d["dr_or_cr"] == "debit_in_account_currency" else "debit", 0
    )
...
def update_reference_in_payment_entry(
    ...
    if payment_entry.book_advance_payments_in_separate_party_account:
        if (
            payment_entry.advance_reconciliation_takes_effect_on
            ...
            reconcile_on = frappe.db.get_value(
                d.against_voucher_type, d.against_voucher, date_field
            )
    ...
    if d.voucher_detail_no:
        existing_row = payment_entry.get(
            "references", {"name": d["voucher_detail_no"]}
        )[0]

        # Update Advance Paid in SO/PO since they are getting unlinked
        if existing_row.get("reference_doctype") in ["Sales Order", "Purchase Order"]:
            update_advance_paid.append(
...
def cancel_exchange_gain_loss_journal(
    ...
    if parent_doc.doctype in [
        ...
        gain_loss_journals = get_linked_exchange_gain_loss_journal(
            referenced_dt=parent_doc.doctype,
        ...
        for doc in gain_loss_journals:
            ...
            if referenced_dt and referenced_dn:
                references = [
                    (x.reference_type, x.reference_name) for x in gain_loss_je.accounts
                ]
                if (
                    len(references) == 2
...
def cancel_common_party_journal(self):
    ...
    if not frappe.db.get_single_value(
        "Accounts Settings", "enable_common_party_accounting"
    ):
        return
...
def update_accounting_ledgers_after_reference_removal(
    ...
    # else:
    reverse_payment_ledger_entries(ref_type, ref_no)
...
def reverse_payment_ledger_entries(ref_type, ref_no):
    ...
    original_entries_si = (
        ...
        .where(
            (ple.voucher_type == ref_type)
            & (ple.voucher_no == ref_no)
            & (ple.delinked == 0)
        )
    ).run(as_dict=True)
    ...
    for record in total_entries:
        ...
        if record["against_voucher_no"] and record["voucher_type"] != "Journal Entry":
...
def unlink_ref_doc_from_payment_entries(
    ref_doc: object = None, payment_name: str | None = None
):
    ...
    update_accounting_ledgers_after_reference_removal(
        ref_doc.doctype, ref_doc.name, payment_name
    )
...
def remove_ref_doc_link_from_jv(
    ...
    linked_jv = (
        [x for x in linked_jv if x == payment_name] if payment_name else linked_jv
    )
...
def remove_ref_doc_link_from_pe(
    ...
    # remove reference only from specified payment
    linked_pe = (
        [x for x in linked_pe if x == payment_name] if payment_name else linked_pe
    )
    ...
    if linked_pe:
        ...
        for pe in linked_pe:
            try:
                ...
                msg = _("There were issues unlinking payment entry {0}.").format(
                    pe_doc.name
                )
                msg += "<br>"
                msg += _("Please cancel payment entry manually first")
                frappe.throw(
                    msg, exc=PaymentEntryUnlinkError, title=_("Payment Unlink Error")
                )

            qb.update(pay).set(
                pay.total_allocated_amount, pe_doc.total_allocated_amount
            ).set(
                pay.base_total_allocated_amount, pe_doc.base_total_allocated_amount
            ).set(
                pay.unallocated_amount, pe_doc.unallocated_amount
            ).set(
                pay.modified, now()
            ).set(
        ...
        frappe.msgprint(
            _("Payment Entries {0} are un-linked").format("\n".join(linked_pe))
        )
        ...
        if outstanding_amount > 0.5 / (10**precision):
            if (
                ...
                and not (
                    ...
                    and outstanding_amount <= max_outstanding
                )
            ):
                continue

            if (
                not d.voucher_type == "Purchase Invoice"
...
def get_account_name(
    ...
    return frappe.db.get_value(
        "Account",
        {
            ...
            or frappe.defaults.get_defaults().currency,
...
def get_companies():
    ...
    return [
        d.name for d in frappe.get_list("Company", fields=["name"], order_by="name")
    ]
...
def get_account_balances(accounts, company):
    ...
    for account in accounts:
        ...
        if (
            ...
            and account["account_currency"] != company_currency
        ):
            account["balance_in_account_currency"] = flt(
                get_balance_on(account["value"], company=company)
            )
...
def update_cost_center(docname, cost_center_name, cost_center_number, company, merge):
    ...
    validate_field_number(
        "Cost Center", docname, cost_center_number, company, "cost_center_number"
    )

    if cost_center_number:
        frappe.db.set_value(
            "Cost Center", docname, "cost_center_number", cost_center_number.strip()
        )
    else:
        frappe.db.set_value("Cost Center", docname, "cost_center_number", "")

    frappe.db.set_value(
        "Cost Center", docname, "cost_center_name", cost_center_name.strip()
    )
...
def get_coa(doctype, parent, is_root=None, chart=None):
    ...
    accounts = build_tree_from_json(
        chart
    )  # returns alist of dict in a tree render-able form
...
def update_gl_entries_after(
    ...
    repost_gle_for_stock_vouchers(
        stock_vouchers, posting_date, company, warehouse_account
    )
...
def repost_gle_for_stock_vouchers(
    ...
    for stock_vouchers_chunk in create_batch(stock_vouchers, GL_REPOSTING_CHUNK):
        ...
        for voucher_type, voucher_no in stock_vouchers_chunk:
            ...
            expected_gle = toggle_debit_credit_if_negative(
            ...
            if expected_gle:
                if not existing_gle or not compare_existing_and_expected_gle(
                    ...
                    voucher_obj.make_gl_entries(
                        gl_entries=expected_gle, from_repost=True
                    )
            else:
                _delete_accounting_ledger_entries(voucher_type, voucher_no)
...
def get_voucherwise_gl_entries(future_stock_vouchers, posting_date):
    ...
    gles = frappe.db.sql(
		...
		where
   		...
   		""".format(
            ", project" if "projects" in frappe.get_installed_apps() else "",
            "%s",
            ", ".join(["%s"] * len(voucher_nos)),
        ),
...
def get_stock_and_account_balance(account=None, posting_date=None, company=None):
    ...
    total_stock_value = get_stock_value_on(
        related_warehouses, posting_date, company=company
    )
    ...
    return (
        flt(account_balance, precision),
        flt(total_stock_value, precision),
        related_warehouses,
    )
...
def get_payment_ledger_entries(gl_entries, cancel=0, reconcile=False):
    ...
    if gl_entries:
        ...
        accounts_with_types = (
            ...
            .where(
                account.account_type.isin(["Receivable", "Payable"])
                & (account.company.isin(companies))
            )
            .run(as_dict=True)
        )
                ...
                if (
                    ...
                    if reconcile == True:
                        ...
                        against_voucher_type = (
                            ...
                            if gle.against_voucher_type != gle.voucher_type
                            else ""
                        )
                        against_voucher_no = (
                            gle.against_voucher
                            if gle.against_voucher != gle.voucher_no
                            else ""
                        )
                    ple = frappe._dict(
                        doctype="Payment Ledger Entry",
...
def create_payment_ledger_entry(
    ...
    if gl_entries:
        ple_map = get_payment_ledger_entries(
            gl_entries, cancel=cancel, reconcile=reconcile
        )
...
def update_voucher_outstanding(voucher_type, voucher_no, account, party_type, party):
    ...
    if (
        ...
        # frappe.throw(str(voucher_outstanding))
...
class QueryPaymentLedger:
    ...
    def query_for_outstanding(self):
        ...
        if self.voucher_no:
            ...
            filter_on_against_voucher_no.append(
                ple.against_voucher_no.like(f"%{self.voucher_no}%")
            )
        ...
        if self.limit and self.get_invoices:
            common_filter_dict = {}
            for i, criterion in enumerate(self.common_filter):
                if isinstance(criterion, Criterion):
                    common_filter_dict[f"filter_{i}"] = str(criterion)
            ...
            outstanding_vouchers = (
                ...
                .select(
                    ...
                    Sum(ple.amount_in_account_currency).as_(
        ...
        query_voucher_amount = (
            ...
            .where(
                Criterion.any(
                    [ple.against_voucher_no != "", ple.against_voucher_no.isnotnull()]
                )
            )
            # .where(Criterion.all(filter_on_voucher_no))
            .where(Criterion.all(self.common_filter))
        ...
        query_voucher_outstanding = (
            ...
            .where(
                Criterion.any(
                    [ple.against_voucher_no.isnull(), ple.against_voucher_no == ""]
                )
            ...
            .having(
                Sum(ple.amount_in_account_currency)
        ...
        self.cte_query_voucher_amount_and_outstanding = (
            ...
            .on(
                (
                    AliasedQuery("vouchers").account
                    == AliasedQuery("outstanding").account
                )
                & (
                    AliasedQuery("vouchers").voucher_type
                    == AliasedQuery("outstanding").voucher_type
                )
                & (
                    AliasedQuery("vouchers").voucher_no
                    == AliasedQuery("outstanding").voucher_no
                )
                & (
                    AliasedQuery("vouchers").party_type
                    == AliasedQuery("outstanding").party_type
                )
            ...
            .select(
                ...
                Table("vouchers").amount_in_account_currency.as_(
                    "invoice_amount_in_account_currency"
                ),
                Table("outstanding").amount.as_("outstanding"),
                Table("outstanding").amount_in_account_currency.as_(
                    "outstanding_in_account_currency"
                ),
                (Table("vouchers").amount - Table("outstanding").amount).as_(
                    "paid_amount"
                ),
                (
...
def run_ledger_health_checks():
    ...
    if health_monitor_settings.enable_health_monitor:
        period_end = getdate()
        period_start = add_days(
            period_end, -abs(health_monitor_settings.monitor_for_last_x_days)
        )
        ...
        if health_monitor_settings.general_and_payment_ledger_mismatch:
            for x in health_monitor_settings.companies:
                ...
                gl_pl_comparison = frappe.get_doc(
                    "Report", "General and Payment Ledger Comparison"
                )
